...
import { Link2, Plus, RefreshCw, ChevronRight, Send, Lock } from 'lucide-react'
import {
  linksListar, linksTags, linksProxima,
  type LinkItem, type LinkDominiosPainel, type OrdemLista, type LinkEstado,
} from '../../lib/linksDb'
import { Empty } from '../../components/Empty'
import { SkeletonList } from '../../components/Skeleton'
import { toast } from '../../lib/toast'
import { n, quando, Ajuda, ESTADO_LINK, Sparkline, UrlCurta, copiarComToast } from './comum'
...
// dos ultimos 7 dias em cada linha. O rodizio de dominio (o que a tela
// antiga punha no topo) so aparece como acao na linha quando ha 2+ raizes
// no rodizio, porque com uma so nao existe rodizio nenhum.
// =====================================================================
export function Lista({ doms, onAbrir }: Props) {
  ...
  async function copiarParaDisparo(l: LinkItem) {
    if (copiando) return
    setCopiando(l.id)
    try {
      // marca a entrega no banco e avanca o rodizio: e a URL que vai pros grupos
      const r = await linksProxima(l.id)
      const ok = await copiarComToast(r.url, 'URL do rodízio')
      if (ok && r.aviso) toast(r.aviso)
    } catch (e) {
    ...
    <>
          ...
          <div className="lk-row" key={l.id}>
            <button className="lk-main" onClick={() => onAbrir(l.id)} aria-label={`Abrir ${l.nome}`}>
              <div className="lk-title">
                ...
                {l.sem_braco && (l.destinos?.length ?? 0) > 1 && <span className="badge b-erro">sem braço</span>}
              </div>
              {l.tags.length > 0 && (
              ...
              <div className="lk-meta">
                {(l.destinos?.length ?? 0) > 1
                  ? `${l.destinos!.length} destinos, ${l.divisao === 'pessoa' ? 'por pessoa' : 'por clique'}`
                  : (l.destinos?.[0]?.url ?? '').replace(/^https?:\/\//, '').slice(0, 60)}
            ...
            <div className="lk-stats">
              ...
              {l.pessoas_7d != null && <div className="lk-num"><b>{n(l.pessoas_7d)}</b><span>pessoas</span></div>}
            </div>

            <div className="lk-acoes">
              {rodizio && (
                <button className="btn ghost sm" onClick={() => copiarParaDisparo(l)} disabled={copiando === l.id}
                  title="Copia a URL do próximo domínio do rodízio e marca a entrega">
                  {copiando === l.id ? <i className="spin" /> : <Send size={13} />}Copiar pra disparo
                </button>
              )}
      ...
      {links && links.length > 0 && (
        <Ajuda titulo="o que cada número da linha quer dizer">
          ...
          <b>Pessoas</b> só aparece quando o link tem cookie de visitante; enquanto não tem, a coluna
          fica de fora em vez de mostrar zero.<br />
